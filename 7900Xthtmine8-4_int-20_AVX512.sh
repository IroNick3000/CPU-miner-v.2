...
# Настройки майнера
WAL=4CY4Cibhs2NQhjXx7DmcF3jJHso96Dxtih
WKN=11_7900X_int20_AVX512
PW="d=0.2"  #Рекомендуемые значения 0.2 на 10C/s, дает стабильный хеш на пуле, для 1C/s 0.02

# Интенсивность
COUNT=6     # Кол-во экземпляров
...
BASE_CMD="nice -n $INT java --add-modules jdk.incubator.vector -XX:UseAVX=512 -Xms2g -Xmx2g -XX:MaxMetaspaceSize=128m -XX:+AlwaysPreTouch -XX:+UseNUMA -XX:+UseParallelGC -XX:+UseSuperWord -XX:+UseInlineCaches -XX:+OptimizeStringConcat -XX:+DoEscapeAnalysis -XX:-TieredCompilation -Xss256k -Djava.awt.headless=true -jar jtminerV0.9ravx512.jar -u ${WAL}.${WKN} -h tht.mine-n-krush.org -p $PW -P 5001 -t $THR"

#Отладчик
log_path="/mnt/ramlogdisk" # Если отказываться от ramdiskа, то изменить на /var/log
log_pattern="mine-*.log"  # Название логов майнера, пока CONST
...
if [ "$ramdisk" = true ]; then
    ...
    mount -t tmpfs -o size=2G tmpfs /mnt/ramlogdisk
else
...
for i in $(seq 0 $((COUNT - 1))); do
    ...
    screen -L -Logfile /mnt/ramlogdisk/"mine-$((i+1))".log -dmS "$SESSION" bash -c "$BASE_CMD"
    sleep "$startlat"
done
